...
const validateSession = (req, res, next) => {
  ...
  if (!token) {
    //! means not, if no token provided say no token provided
    return res.status(403).send({ auth: false, message: "No token provided" });
  } else {
    jwt.verify(token, process.env.JWT_SECRET, (err, decodeToken) => {
      //verifies the token/magicband and decrypts it
      console.log("decodeToken --> ", decodeToken);
      if (!err && decodeToken) {
        //no error when decrypting, unencrypt it and search database
        User.findOne({
          where: {
            //where are you searching? looking for YOU
            id: decodeToken.id,
          },
        })
          .then((user) => {
